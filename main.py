...
async def get_commodity_details(com_id: str, days: int = 7):
    try:
        ...
        if isinstance(historical_data.columns, __import__('pandas').MultiIndex):
...
async def websocket_endpoint(websocket: WebSocket, com_id: str, days: int = 7):
    ...
    try:
        while True:
            ...
            if isinstance(live_data_df.columns, __import__('pandas').MultiIndex):
                live_data_df.columns = live_data_df.columns.get_level_values(0)
